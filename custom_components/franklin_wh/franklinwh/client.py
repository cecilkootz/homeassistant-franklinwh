...
class Client(HttpClientFactory):
    ...
    async def get_stats(self) -> Stats:
        ...
        battery_charge_total = float(data.get("kwh_fhp_chg", 0.0))
        grid_export_total = float(data.get("kwh_uti_out", 0.0))

        grid_to_battery, solar_to_battery = self._normalize_energy_pair_to_total(
            float(data.get("gridChBat", 0.0)),
            float(data.get("soChBat", 0.0)),
            battery_charge_total,
        )
        solar_to_grid, battery_to_grid = self._normalize_energy_pair_to_total(
            float(data.get("soOutGrid", 0.0)),
            float(data.get("batOutGrid", 0.0)),
            grid_export_total,
        )
    ...
    @staticmethod
    def _normalize_energy_pair_to_total(
        first: float, second: float, total: float
    ) -> tuple[float, float]:
        """Normalize pair values that may be reported in kWh or Wh.

        Some firmware/API paths return these breakdown fields in kWh while
        others return Wh-equivalent values. Pick the scale that best matches
        the corresponding known total for this poll.
        """
        raw_sum = first + second
        scaled_sum = raw_sum / 1000.0

        if total > 0:
            raw_error = abs(raw_sum - total)
            scaled_error = abs(scaled_sum - total)
            if scaled_error + 1e-6 < raw_error:
                return first / 1000.0, second / 1000.0
            return first, second

        # Fallback when the reference total is zero/unavailable.
        if max(abs(first), abs(second)) >= 1000:
            return first / 1000.0, second / 1000.0
        return first, second
